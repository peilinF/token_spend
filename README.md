...
| cursor | `cursor.com/api/dashboard/get-filtered-usage-events` | 自动从 Chrome 提取会话 cookie 调用网页版接口 |

### Cursor 凭据自动化

Cursor 不在本地存 token 数据，TokenSpend 会：

1. 复制 Chrome 的 Cookies 数据库到临时目录（避开文件锁）
2. 从钥匙串读取 `Chrome Safe Storage` 密钥（**首次弹一次授权框，点「始终允许」后永久静默**）
3. AES-128-CBC 解密出 `WorkosCursorSessionToken`（仅存内存，不落盘）
4. 删除临时副本，带 cookie 调用 usage API

cookie 过期后详情面板提示「请在 Chrome 登录 cursor.com」，重新登录即自动恢复。
...
| 任务 | 频率 | 说明 |
|---|---|---|
| opencode/codex 增量 | 30s + 实时轮询 5s | 毫秒级增量，无 IO 压力 |
| cursor API | 300s | 每小时 ≤12 次请求，失败指数退避至 60min |
...
- cursor 的 token 速率显示依赖 API 同步（5 分钟粒度），脉冲点才是实时的
...
│   ├── CodexSource.swift
│   └── CursorSource.swift  # 含 Chrome cookie 解密 + API 客户端
└── UI/
